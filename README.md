...
*   **Dynamic Token Tracking & Pinning**: Automatically follows your active or selected token. Toggle between three positioning modes: **Attached** (dynamic token tracking), **Pinned** (fixed offset relative to token top-left, clamped to page), and **Detached** (floating screen coordinates).
*   **Dual-Tab Filtering**:
...
*   **Floating / Detached Mode**: Drag the HUD anywhere on your screen—it will smoothly glide at 60fps and save its position. Click the anchor button to cycle positioning modes.
...
*   **Configurable Placement Preference Order**: Select your preferred sequence of sides around the token (`Top → Bottom → Left → Right`, `Bottom → Top → Right → Left`, etc.) where the HUD will attempt to attach.
...
*   **Click the Anchor / Pin Icon**: Cycles through HUD positioning modes:
    *   `<i class="fas fa-link"></i>` **Attached**: Dynamic token tracking based on your placement preference order.
